...
func (s *EditorScreen) Reveal(p EditorPosition) bool {
...
func (s *EditorScreen) centerIfOffscreen() {
	if s.w < 1 || s.h < 1 {
		return
	}
	row := s.curY
...
func (s *EditorScreen) SelectRange(r EditorRange) bool {
...
// EditorEdit is one range replacement in a set applied together.
...
func (s *EditorScreen) ApplyEdits(edits []EditorEdit) bool {
	if len(edits) == 0 {
		return false
	}
	ordered := make([]EditorEdit, len(edits))
	copy(ordered, edits)
	...
	caret := EditorPosition{Line: s.curY, Column: s.curX}
	s.cancelCompletionSession()
	s.editAtomic(func() {
		s.clearSelection()
