...
func TestEditorHCaretBandIsKeyboardOnly(t *testing.T) {
	press := func(mod tea.KeyMod) (*Screen, *core.Shared, int) {
		s, sh := newEditor(Opts{})
...
func dragTick(s *Screen, sh *core.Shared) core.Action {
...
func TestEditorDragScrollStops(t *testing.T) {
	for name, end := range map[string]func(*Screen, *core.Shared){
