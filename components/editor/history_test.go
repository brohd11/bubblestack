...
func undoEditor(s *Screen) { s.key(nil, keyMsg("ctrl+z")) }
func redoEditor(s *Screen) { s.key(nil, keyMsg("ctrl+y")) }
...
func TestEditorStructuredEnterIsOneUndoStep(t *testing.T) {
	...
	s, _ := newEditor(Opts{Path: "structured", ResolveLanguage: resolver})
	s.setContent("item")
...
func TestEditorUndoRedoHelpBindings(t *testing.T) {
	var all []string
	for _, binding := range New(Opts{}).HelpBindings() {
