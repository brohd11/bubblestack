...
func (s *Screen) historyState() editorState {
	return editorState{
...
func (s *Screen) beginHistory() *editorHistoryEntry {
	if s.activeEdit != nil {
		return nil
	}
...
func (s *Screen) finishHistory(entry *editorHistoryEntry) bool {
	if entry == nil {
		return false
	}
	s.activeEdit = nil
...
func (s *Screen) textBetween(start, end textPos) string {
	if start == end {
		return ""
	}
	if start.y == end.y {
...
func (s *Screen) insertRunes(rs ...rune) {
	if len(rs) == 0 {
		return
	}
	start := textPos{s.curY, s.curX}
