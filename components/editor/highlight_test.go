...
func trackedHighlighterLanguage(created *[]*restartHL, restart []int) LanguageResolver {
	return func(string) *LanguageConfig {
...
func highlighterLanguage(path string) *LanguageConfig {
	if strings.HasSuffix(strings.ToLower(path), ".lit") {
		return &LanguageConfig{NewHighlighter: func() Highlighter { return &countingHL{} }}
	}
	return nil
}

func TestEditorLanguageHighlighterResolution(t *testing.T) {
	plain := New(Opts{Path: "notes.txt", ResolveLanguage: highlighterLanguage})
	if plain.hl != nil {
		t.Fatal("a nil language profile should leave the editor plain")
	}

	a := New(Opts{Path: "one.LIT", ResolveLanguage: highlighterLanguage})
	b := New(Opts{Path: "two.lit", ResolveLanguage: highlighterLanguage})
...
func TestEditorConfiguredHighlightRender(t *testing.T) {
	styled, _ := newEditor(Opts{Path: "x.lit", ResolveLanguage: highlighterLanguage})
	styled.setContent("alpha\na\tb")
	plain, _ := newEditor(Opts{})
...
func TestEditorHighlighterMismatchFallsBack(t *testing.T) {
	s, _ := newEditor(Opts{Highlighter: brokenHL{}})
...
func TestEditorHighlighterReparseCache(t *testing.T) {
	h := &countingHL{}
	s, sh := newEditor(Opts{Highlighter: h})
	s.setContent("alpha\nbeta")
	s.View(sh)
	s.View(sh)
	if h.parses != 1 {
...
func TestEditorHighlighterUsesBoundedRestartHint(t *testing.T) {
	...
	var created []*restartHL
	s, sh := newEditor(Opts{Path: "x.lit", ResolveLanguage: trackedHighlighterLanguage(&created, restarts)})
...
func TestEditorHighlighterRejectsPreviousLanguageSnapshot(t *testing.T) {
	...
	s, _ := newEditor(Opts{Path: "old.lit", ResolveLanguage: resolve})
