...
func TestEditorRightPressOffTextOpensNothing(t *testing.T) {
	...
	bar, shBar := newEditor(Opts{ContextMenu: true})
...
func TestEditorContextMenuRows(t *testing.T) {
	...
	empty, shEmpty := newEditor(Opts{
		ContextMenu:  true,
		ContextItems: func(*core.Shared) []components.MenuItem { return nil },
	})
	...
	extra, shExtra := newEditor(Opts{
		ContextMenu:  true,
		ContextItems: func(*core.Shared) []components.MenuItem { return []components.MenuItem{{Label: "Toggle wrap"}} },
	})
...
func openContextMenu(t *testing.T, termH int, content string, setup func(*Screen),
	x, row int) (*components.MenuScreen, *Screen, int) {
	t.Helper()
	s := New(Opts{ContextMenu: true})
...
func TestEditorContextMenuAnchorAbsolute(t *testing.T) {
	pane, shPane := newPaneEditor(Opts{ContextMenu: true})
	...
	free, shFree := newEditor(Opts{ContextMenu: true})
...
func TestEditorContextMenuClearsTheText(t *testing.T) {
	...
	if x, y := menu.OverlayPos(0, 0); x != 3 || y != textTop+2 {
...
func TestEditorContextMenuFlipsAboveTheText(t *testing.T) {
	...
	// Height comes off the real render rather than the menu's internals: the type's
	// contract is that placement and View agree to the cell, so measuring the box is
	// the same number and asserts the agreement too.
	x, y := menu.OverlayPos(0, 0)
	h := lipgloss.Height(menu.View(nil))
...
func TestEditorPasteIsAddressed(t *testing.T) {
	mine, sh := newEditor(Opts{ContextMenu: true})
	mine.setContent("abc")
	other, _ := newEditor(Opts{ContextMenu: true})
...
func TestEditorPasteReadSeam(t *testing.T) {
	s, _ := newEditor(Opts{ContextMenu: true})
...
func TestEditorContextMenuThroughRouter(t *testing.T) {
	...
	if _, still := tm.(core.Router).Top().(*components.MenuScreen); still {
	...
	// A right press while the menu is up dismisses it (components.MenuScreen's non-left branch).
	tm, _ = tm.Update(rightPress(2, sh.BodyY()+s.titleH()))
	if _, ok := tm.(core.Router).Top().(*components.MenuScreen); !ok {
