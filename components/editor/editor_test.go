...
func newEditor(opts Opts) (*Screen, *core.Shared) {
	s := New(opts)
...
func typeRunes(s *Screen, rs ...rune) {
	s.key(nil, keyMsg(string(rs)))
}

func buffer(s *Screen) string {
...
func selectRange(s *Screen, y1, x1, y2, x2 int) {
...
func pasteText(s *Screen, text string) {
...
func TestEditorPasteSkipsEnterHandler(t *testing.T) {
	resolver := func(string) *LanguageConfig {
		return &LanguageConfig{OnEnter: func(EnterContext) (EnterAction, bool) {
			return EnterAction{Prefix: "> "}, true
		}}
	}
	s, _ := newEditor(Opts{Path: "notes.md", ResolveLanguage: resolver})
	if s.onEnter == nil {
...
// in opts.
func newPaneEditor(opts Opts) (*Screen, *core.Shared) {
	s := New(opts)
	s.SetEmbedded(true)
	sh := core.NewShared(nil)
	s.SetSize(sh, 40, 10)
...
func TestEditorBorderedView(t *testing.T) {
	s, sh := newPaneEditor(Opts{Title: "notes.md", Border: true})
	v := s.View(sh)
	...
	plain, sh2 := newEditor(Opts{Title: "notes.md"})
	pv := plain.View(sh2)
...
func TestEditorSizeInsets(t *testing.T) {
	...
	pane, _ := newPaneEditor(Opts{}) // embedded, unbordered: gutter + title bar
	if pane.w != 39 || pane.h != 10-pane.titleH() {
		t.Fatalf("pane 40x10 → %dx%d, want 39x%d", pane.w, pane.h, 10-pane.titleH())
	}

	framed, _ := newPaneEditor(Opts{Border: true}) // frame + gutter
...
func TestEditorPaneClick(t *testing.T) {
	press := func(s *Screen, sh *core.Shared, x, y int) {
		s.Update(sh, tea.MouseClickMsg{X: x, Y: y, Button: tea.MouseLeft})
	}

	t.Run("unbordered", func(t *testing.T) {
		s, sh := newPaneEditor(Opts{})
		s.setContent("hello\nhi\nworld")
	...
	t.Run("bordered", func(t *testing.T) {
		s, sh := newPaneEditor(Opts{Border: true})
		s.setContent("hello\nhi\nworld")
...
func TestEditorUnfocusedRender(t *testing.T) {
	...
	s, sh := newPaneEditor(Opts{Title: "notes.md"})
	s.setContent("hello\nworld")
	if !s.focused {
...
func TestEditorLoadAndDirty(t *testing.T) {
	...
	missing, sh2 := newEditor(Opts{Path: filepath.Join(dir, "nope.txt")})
...
func TestEditorSaveKey(t *testing.T) {
	...
	t.Run("keeps the caret and the view", func(t *testing.T) {
		s, sh := newEditor(Opts{Path: filepath.Join(dir, "caret.txt")})
	...
	t.Run("renaming re-picks the highlighter", func(t *testing.T) {
		s, _ := newEditor(Opts{Path: filepath.Join(dir, "plain.txt"), ResolveLanguage: highlighterLanguage})
		if s.hl != nil {
		...
		explicit := &countingHL{}
		s2, _ := newEditor(Opts{Path: filepath.Join(dir, "a.lit"), ResolveLanguage: highlighterLanguage, Highlighter: explicit})
...
func wheel(s *Screen, sh *core.Shared, btn tea.MouseButton) {
...
func altWheel(s *Screen, sh *core.Shared, btn tea.MouseButton) {
...
func TestEditorScrollbar(t *testing.T) {
	...
	s, _ := newEditor(Opts{}) // standalone: no gutter, the bar is the last cell
	s.setContent("a\nb\nc")
...
func TestEditorSaveAsAnchor(t *testing.T) {
	s, sh := newEditor(Opts{}) // standalone 80x20
	x, y, w := s.saveAsEdit(sh).Anchor()
	if x != 0 || w != sh.Width() {
		t.Fatalf("standalone anchor = (%d, w %d), want (0, %d)", x, w, sh.Width())
	}
	if want := sh.BodyY() + s.paneH - 2; y != want {
		t.Fatalf("standalone y = %d, want %d (one row above the prompt row)", y, want)
	}

	pane, psh := newPaneEditor(Opts{}) // embedded 40x10: gutter + title bar
	pane.SetPaneOrigin(31, 4)
	x, y, w = pane.saveAsEdit(psh).Anchor()
	if pane.paneW() != 40 {
		t.Fatalf("paneW = %d, want the pane's full 40", pane.paneW())
	}
	if x != 31 || w != 40 {
		t.Fatalf("embedded anchor = (%d, w %d), want (31, 40)", x, w)
	}
	if want := 4 + pane.paneH - 2; y != want {
		t.Fatalf("embedded y = %d, want %d", y, want)
	}
	...
	_, y, _ = pane.saveAsEdit(psh).Anchor()
	if want := 4 + pane.paneH - 2; y != want {
		t.Fatalf("embedded y with search = %d, want pane-bottom anchor %d", y, want)
	}
}
...
func assertRowGeometry(t *testing.T, s *Screen, ctx string) {
	t.Helper()
	rows := strings.Split(s.body(), "\n")
	if len(rows) != s.h {
...
func TestEditorWrapCaretHighlighted(t *testing.T) {
	s, _ := newEditor(Opts{Highlighter: &countingHL{}})
