...
func TestEditorAutoIndentFollowsExtension(t *testing.T) {
	...
	yaml, _ := newEditor(Opts{Path: "conf.soft", ResolveLanguage: resolver})
	yaml.setContent("a\nb")
	...
	txt, _ := newEditor(Opts{Path: "notes.txt"})
	txt.setContent("a\nb")
...
func TestEditorSaveAsRepicksIndentUnit(t *testing.T) {
	...
	s, _ := newEditor(Opts{Path: "notes.txt", ResolveLanguage: resolver})
...
func TestEditorExplicitIndentWidthSurvivesRename(t *testing.T) {
	...
	s, _ := newEditor(Opts{Path: "notes.txt", ResolveLanguage: resolver, Indent: IndentSpaces, IndentWidth: 3})
	s.applySaveName("conf.soft")
	s.setContent("a\nb")
	selectRange(s, 0, 0, 1, 1)
	s.key(nil, tabKey)
...
func TestEditorCycleIndentMode(t *testing.T) {
	resolver := func(string) *LanguageConfig { return &LanguageConfig{IndentSpaces: 2} }
	s, _ := newEditor(Opts{Path: "conf.soft", ResolveLanguage: resolver})
	s.setContent("a\nb")
...
func TestEditorIndentTabModeOverridesProfile(t *testing.T) {
	resolver := func(string) *LanguageConfig { return &LanguageConfig{IndentSpaces: 2} }
	s, _ := newEditor(Opts{Path: "conf.soft", ResolveLanguage: resolver, Indent: IndentTab})
	s.setContent("a\nb")
	selectRange(s, 0, 0, 1, 1)
	s.key(nil, tabKey)
	if got, want := buffer(s), "\ta\n\tb"; got != want {
