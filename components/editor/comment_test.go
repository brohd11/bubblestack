...
func commentEditor(t *testing.T, path, content string) *Screen {
	t.Helper()
	s, _ := newEditor(Opts{Path: path, ResolveLanguage: testLanguage})
	s.setContent(content)
	return s
}

func toggle(s *Screen) { s.key(nil, keyMsg("ctrl+_")) }
...
func TestCommentBindingAdvertisesBothKeys(t *testing.T) {
	s, _ := newEditor(Opts{Path: "commented.slash", ResolveLanguage: testLanguage})
