...
// EditorCompletionStop describes one snippet tab stop using rune offsets into
// EditorCompletionEdit.Text. Index zero is the final caret. Positive indices are
...
// EditorCompletionEdit is an atomic, externally supplied completion replacement.
...
type EditorCompletionEdit struct {
	Range              EditorRange
	Text               string
...
func (s *EditorScreen) ApplyCompletion(edit EditorCompletionEdit) bool {
...
func completionPositions(start textPos, value string, supplied []EditorCompletionStop) (
	[]textPos, []editorCompletionStop, *EditorCompletionStop, bool,
) {
	runes := []rune(value)
	...
	positive := make([]EditorCompletionStop, 0, len(supplied))
	var final *EditorCompletionStop
...
func (s *EditorScreen) cancelCompletionSession() { s.completion = nil }

func (s *EditorScreen) selectCompletionStop(index int) {
...
func (s *EditorScreen) advanceCompletionStop() {
	if s.completion == nil {
		return
	}
...
func (s *EditorScreen) handleCompletionKey(k string, m tea.KeyPressMsg) bool {
	if s.completion == nil {
		return false
	}
	switch k {
...
func (s *EditorScreen) rebaseCompletionEdit(start, end textPos, inserted string) {
	if s.completion == nil {
		return
	}
