...
func completionEditor(t *testing.T, text string) *EditorScreen {
	t.Helper()
	s, _ := newEditor(EditorOpts{ResolveLanguage: func(string) *EditorLanguageConfig {
		return &EditorLanguageConfig{AutoClosingPairs: []EditorPair{
...
func TestEditorCompletionPairsTrailingOpenerAtomically(t *testing.T) {
	...
	if !s.ApplyCompletion(EditorCompletionEdit{
		Range: EditorRange{End: EditorPosition{Column: 2}}, Text: "my_func(", PairTrailingOpener: true,
	}) {
	...
	if got := s.CursorPosition(); got != (EditorPosition{Column: 8}) {
	...
	if !s.ApplyCompletion(EditorCompletionEdit{
		Range: EditorRange{End: EditorPosition{Column: 2}}, Text: "my_func(", PairTrailingOpener: true,
	}) || s.Text() != "my_func()" || s.CursorPosition() != (EditorPosition{Column: 8}) {
		t.Fatalf("existing closer was not reused: text=%q caret=%+v", s.Text(), s.CursorPosition())
	}

	s.SetText("value")
	if !s.ApplyCompletion(EditorCompletionEdit{
		Range: EditorRange{End: EditorPosition{Column: 5}}, Text: "value\"", PairTrailingOpener: true,
	}) || s.Text() != "value\"" {
...
func TestEditorCompletionSnippetStopsRebaseThroughEdits(t *testing.T) {
	...
	if !s.ApplyCompletion(EditorCompletionEdit{
		Range: EditorRange{End: EditorPosition{Column: 2}},
	...
	if got := s.CursorPosition(); got != (EditorPosition{Line: 1, Column: 2}) || s.completion != nil {
...
func TestEditorCompletionSnippetEscapeAndValidation(t *testing.T) {
	...
	bad := EditorCompletionEdit{
		Range: EditorRange{End: EditorPosition{Column: 1}}, Text: "value",
		Stops: []EditorCompletionStop{{Index: 1, Start: 0, End: 1}, {Index: 1, Start: 2, End: 3}},
	}
	...
	if !s.ApplyCompletion(EditorCompletionEdit{
		Range: EditorRange{End: EditorPosition{Column: 1}}, Text: "value",
		Stops: []EditorCompletionStop{{Index: 1, Start: 0, End: 5}},
	}) {
