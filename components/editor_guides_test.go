...
func TestEditorIndentGuidesUseLiveUnit(t *testing.T) {
	...
	s, _ := newEditor(EditorOpts{ResolveLanguage: resolver, IndentGuides: true})
	s.setContent("    value\n   partial\n\t tabbed")
	s.SetFocused(false)
...
func TestEditorIndentGuidesFollowLanguageAndHighlighting(t *testing.T) {
	resolver := func(path string) *EditorLanguageConfig {
		...
		return &EditorLanguageConfig{IndentSpaces: spaces}
	}
	hl := &countingHL{}
	s, _ := newEditor(EditorOpts{
...
func TestEditorIndentGuidesKeepWindowGeometry(t *testing.T) {
	s, _ := newEditor(EditorOpts{
		IndentGuides:    true,
		ResolveLanguage: func(string) *EditorLanguageConfig { return &EditorLanguageConfig{IndentSpaces: 2} },
	})
	s.SetSize(nil, 8, 4)
	s.setContent("      value")
